...
class Evaluator(object):
	def __init__(self, plant, normValue = 1):
		...
		self.powerUsageWeight = 0.0
		self.craneIdleTimesWeight = 0.2
		self.orderDeadlinesWeight = 0.4
		self.zincUtilizationWeight = 0.4
	...
	def evaluate(self, solutions):
		...
		"""
		assert type(solutions) != type(None)
	...
	def evalCraneIdleTimes(self, schedule, finishTimes):
		...
		for finishTime in finishTimes:
			...
			penalty -= finishTime[1] - \
				...
				order.recipe.minProcTime(self.plant, machineName)
		return penalty

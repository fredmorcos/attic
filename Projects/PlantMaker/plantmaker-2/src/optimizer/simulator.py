from optimizer.simulatorutils import *
...
class Simulator(object):
	...
	def checkTraverseNodeFinishTime(self, nodeIndex, node, t):
		...
		for j, o in enumerate(node.orders): 
			...
			if o[1] > 0:
				...
				if any([z in machine.setOfBreaks for z in 
					  range(t, t + o[0].recipe[machine.name])]) == False:
	...
	def checkScheduleEnterTimes(self, schedule, t):
		...
		for i, s in enumerate(schedule):
			...
			if s[2] <= t:
				...
				if any([z in machine.setOfBreaks for z in
					  range(t, t + s[0].recipe[machine.name])]) == False: 
	...
	def simulate(self, inschedule):
		...
		schedule.sort(key = itemgetter(2))
		...
		# schedule.sort(key = lambda a, b: cmp(b[0].currentMachine, a[0].currentMachine))
		schedule.sort(key = lambda a: a[0].currentMachine, reverse = True)

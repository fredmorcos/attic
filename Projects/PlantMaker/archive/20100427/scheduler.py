...
class Scheduler(object):
	...
	def addPrecedenceConstraint(self, enterVar, order, machineIndex):
		...
		currentMachine = self.plant.machines[machineIndex]
		if prevMachine.quantity <= self.plant.machines[machineIndex].quantity \
	...
	def addTotalTimeVar(self, order):
		...
		"""
		var = str(order.id) + "-total"
		lastMachine = self.plant.machines[-1]
		self.problem.addVariable(var, range(order.deadline - EndMargin,
			order.deadline + EndMargin))
		self.problem.addConstraint(lambda x, y, yt: x == y + yt,
	...
	def addOrderEnterTimeAtMachineVar(self, order, machineName, machineIndex):
		...
		machineStart = (order.deadline + EndMargin) - \
			order.recipe.calcMinProcTime(machineName) - MachineMargin
		machineEnd = machineStart + (2 * MachineMargin)
	...
	def run(self):
		...
		for machineIndex, machine in enumerate(self.plant.machines):
			breaks = machine.setOfBreaks()
			if len(breaks) != 0:
				for order in self.orderList.orders:
					var = self.createEnterTimeVarName(order, machine)
					self.problem.addConstraint(NotInSetConstraint(breaks),
						[var])
	...
	def printSolutions(self):
		...
		for s in sols:
			...
			for j in items:
				if j[0][0:1] != str(i):
					if "enter" in j[0] or "total" in j[0]:
						print j,
				else:
					print "\n",
					print "Order no:", i

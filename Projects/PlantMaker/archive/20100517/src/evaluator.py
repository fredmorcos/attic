...
class Evaluator(object):
	def __init__(self, plant):
		...
		self.zincBreakTime = 25
		
		self.powerUsageWeight = 0.1
		self.craneIdleTimesWeight = 0.1
		self.orderDeadlinesWeight = 0.1
		self.zincUtilizationWeight = 0.7
	...
	def run(self, solution):
		...
		res += self.evalDeadlines(solution.finishTimes) * self.orderDeadlinesWeight
		res += self.evalCraneIdleTimes(solution.schedule, solution.finishTimes) * self.craneIdleTimesWeight
		res += self.evalPowerUsage(solution.schedule) * self.powerUsageWeight
		res += self.evalZincUtilization(solution.schedule) * self.zincUtilizationWeight
	...
	def evalZincUtilization(self, schedule):
		...
		for i, enterTime in enumerate(zincEnterTimes[:-1]):
			penalty += abs((zincEnterTimes[i + 1][2] - enterTime[2]) - self.zincBreakTime)

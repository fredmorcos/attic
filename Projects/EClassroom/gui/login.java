...
public class login extends JFrame implements ActionListener {
	...
	engine				db_engine 			= new engine("../sql/elearning.db");
	main_window			db_window;
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (action_event.equalsIgnoreCase("Cancel")) {
			...
			boolean check = db_engine.do_login(username_text.getText(), 
							password_text.getText(),
							type_combo.getSelectedItem().toString().toLowerCase());
			if (check == true) {
				status_label.setText("Logged in! Loading...");
				db_window = new main_window(username_text.getText(), 
											type_combo.getSelectedItem().toString(),
											db_engine);
				this.dispose();
			}
			else {
				status_label.setText("Donkey!");
			}
		}

	...
	public void actionPerformed(ActionEvent event){
		if(event.getActionCommand().equals("Connect")) {
			try {
				...
				out_data.writeBytes(name_text.getText() + "\n");
				String authentication = in_data.readLine();
				if(authentication.equals("OK")){
					System.out.println("connection established");
					thread_client client_thread = new thread_client();
					client_thread.start();
					chat_window = new client_chat_gui(name_text.getText(), client_socket);
					chat_window.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
					this.setVisible(false);
				}else{
					System.out.println("Nick Name is not avaialabe now. Try to chose another");
					name_text.setText("");
				}
			}
			catch (Exception e) {
	...
	public void actionPerformed(ActionEvent event) {
		try {
			if(event.getActionCommand().equals("Send")) {
				...
				!(message_text.getText().toLowerCase().equals("end")) && 
				!(message_text.getText().toLowerCase().equals("bye"))) {
				...
				else if (message_text.getText().toLowerCase().equals("end") || message_text.getText().toLowerCase().equals("bye")) {

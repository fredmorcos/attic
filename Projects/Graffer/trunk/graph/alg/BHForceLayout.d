...
/**
 * \brief Implementation of the Barnes-Hut algorithm for automatic graph layout 
 * in O(n log n).
 */
class BHForceLayout: TimeForceLayout {
public:
	double	theta = 0.5;		/**< Barnes-Hut theta. */
	...
	void run() {
		...
		if (MT) {
			auto pool = new ThreadPool!(Vertex*, QuadTree*)
								(nThreads, graph.vertices.length);

			foreach(ref v; graph.vertices)
				pool.append(&repulsion, v, tree);

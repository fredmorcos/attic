...
public:
	this() {
		...
		with (eBox = new HBox(false, 5)) {
			packStart(new Label("Electric Constant"), false, false, 0);
		...
		with (sBox = new HBox(false, 5)) {
			packStart(new Label("Spring Constant"), false, false, 0);
			packStart(sConstSpin, true, true, 0);
		}

		with (sNLCheck = new CheckButton("Natural Length")) {
		...
		with (runButton = new Button(StockID.EXECUTE, true)) {
		}

		with (stopButton = new Button(StockID.STOP, true)) {
			setSensitive(false);
		}

		with (buttonsBox = new HBox(false, 5)) {
			packStart(runButton, false, false, 0);
			packStart(stopButton, false, false, 0);
		}

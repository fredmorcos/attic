...
//	JLabel pic[]=new JLabel[6];
	...
	public boolean allWritten() {
    	for(int i = 0; i < num; i++) {
    		if(names[i].getText().equals("")) return false;
    	}
    	return true;
    }
	...
	public void mouseClicked(MouseEvent e) {
		...
		else if(e.getSource().equals(startBut)&& allWritten()) {
			try {
	    		nameFile = new PrintWriter(new FileOutputStream("names.txt"));
		    	for(int i = 0; i<num; i++) {
		            nameFile.println(names[i].getText());       
		    	}    
		   		nameFile.close();    
		    	dispose();
				new GameFrame();	
				...
				System.out.println("Fatal Error!. Names file cannot be found");
				System.exit(0);
			}
		}

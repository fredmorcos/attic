...
public class Game
{
	...
	public void addPlayerToGame(Player x) {
		//will add player x to the array that will be sent to firstplayer()
		playerArr[x.playerNumber]=x;
	}
	...
	public void startGame() {
		...
		for(int i=0; i<playerArr.length; i++) {
			playerArr[i].myBoard.insertTile((Tile)theWell.draw());
		}
    ...
    public Player firstPlayer(Player[] p){
		...
		for(int i=0; i<x-1; i++){
			if(p[i].myBoard.getTile(0).triple()){
				p2[j]=p[i];
				j++;
			}
		}
		if(p2[0]!=null){
			int max=p2[0].myBoard.getTile(0).totalValue();
			for(j=1; j<p2.length; j++){//p2[j]!=null){
				if(p2[j].myBoard.getTile(0).totalValue()>max){
					max= p2[j].myBoard.getTile(0).totalValue();
					maxplayer=p2[j];
				}
			}
			return maxplayer;
			...
			int max=p[0].myBoard.getTile(0).totalValue();
			for(int i=1; i<p.length; i++){
				if(p[i]==null){
					continue;
				}
				else{
					if(p[i].myBoard.getTile(0).totalValue()>max){
						maxplayer=p[i];
				    }
				}
			}

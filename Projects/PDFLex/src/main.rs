...
enum Error {
  ...
  StrToFloat(std::num::ParseFloatError),
  InvalidNum,
}

type ParseRes<'doc> = Result<Token<'doc>, Error>;
      ...
      *pos += 1;
      ...
      if OCT_DIGITS.contains(&data[*pos]) {
        ...
        *pos += 1;
        ...
        let n = u32::from_str_radix(&esc, 8).map_err(Error::StrToInt)?;
        s.push(std::char::from_u32(n).ok_or(Error::IntToChar)?);
        ...
        return Err(Error::UnsupportedEscapeSeq(char::from(data[*pos])));
      } else if data[*pos] == b'\\' {
        s.push('\\');
      } else if data[*pos] == b'(' {
        s.push('(');
        ...
        data[*pos .. *pos + CRLF.len()] == CRLF
      {
        ...
        return Err(Error::UnrecognizedEscapeSeq(char::from(data[*pos])));
      }
    } else {
...
fn lex_hexstr<'doc>(data: &'doc [u8], pos: &mut usize) -> ParseRes<'doc> {
  ...
  *pos +=1;                     // skip the <
  ...
  if data[*pos] != GT {
    return Err(Error::MissingHexStrClosing);
  }
  ...
  while *pos < data.len() && !WS.contains(&data[*pos]) && !DELIM.contains(&data[*pos]) {
    ...
    if data[*pos] == NUMSIGN {
      ...
      *pos += 1;
      ...
      let n = u32::from_str_radix(&num, 16).map_err(Error::StrToInt)?;
      name.push(std::char::from_u32(n).ok_or(Error::IntToChar)?);
    } else {
...
fn lex_numeric<'doc>(data: &'doc [u8], pos: &mut usize) -> ParseRes<'doc> {
  ...
  if data[*pos] == DOT {
    *pos += 1;
    ...
    let num: &str = std::str::from_utf8(&data[start .. *pos]).map_err(Error::UTF8)?;
    Ok(Token::Real(f64::from_str(num).map_err(Error::StrToFloat)?))
    ...
    *pos += 1;
    ...
    if data[*pos] == DOT {
      *pos += 1;
      ...
      let num: &str = std::str::from_utf8(&data[start .. *pos]).map_err(Error::UTF8)?;
      Ok(Token::Int(i128::from_str(num).map_err(Error::StrToInt)?))
    }
    ...
    *pos += 1;
    ...
    Err(Error::InvalidNum)
  }
}

fn lex_document(data: &[u8]) -> Result<Document, Error> {
...
fn main() -> Result<(), Error> {
  let mut file: File = File::open(Opt::from_args().file).map_err(Error::IO)?;
  let mut data: Vec<u8> = Vec::new();

  file.read_to_end(&mut data).map_err(Error::IO)?;

  let doc = lex_document(&data)?;
  print!("{}", doc);

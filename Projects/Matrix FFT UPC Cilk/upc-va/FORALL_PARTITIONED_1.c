/* --- UPCR system headers --- */ 
#include "upcr.h" 
#include "whirl2c.h"
#include "upcr_proxy.h"
/*******************************************************
 * C file translated from WHIRL Thu Feb  9 07:18:46 2012
 *******************************************************/

/* UPC Runtime specification expected: 3.6 */
#define UPCR_WANT_MAJOR 3
#define UPCR_WANT_MINOR 6
/* UPC translator version: release 2.14.0, built on Oct 25 2011 at 16:40:27, host aphid linux-x86_64/64, gcc v4.2.4 (Ubuntu 4.2.4-1ubuntu4) */
/* Included code from the initialization script */
#include "upcr_geninclude/stdio.h"
#include "upcr_geninclude/math.h"
#include</san1/home/gup/k339970/vector-addition/gettime.h>
#include "upcr_geninclude/stddef.h"
#include</home/ica/k339970/opt/opt/include/upcr_preinclude/upc_bits.h>
#include "upcr_geninclude/stdlib.h"
#include "upcr_geninclude/inttypes.h"
#line 1 "main.w2c.h"
/* Include builtin types and operators */

#ifndef UPCR_TRANS_EXTRA_INCL
#define UPCR_TRANS_EXTRA_INCL
extern int upcrt_gcd (int _a, int _b);
extern int _upcrt_forall_start(int _start_thread, int _step, int _lo, int _scale);
#define upcrt_forall_start(start_thread, step, lo, scale)  \
       _upcrt_forall_start(start_thread, step, lo, scale)
int32_t UPCR_TLD_DEFINE_TENTATIVE(upcrt_forall_control, 4, 4);
#define upcr_forall_control upcrt_forall_control
#ifndef UPCR_EXIT_FUNCTION
#define UPCR_EXIT_FUNCTION() ((void)0)
#endif
#if UPCR_RUNTIME_SPEC_MAJOR > 3 || (UPCR_RUNTIME_SPEC_MAJOR == 3 && UPCR_RUNTIME_SPEC_MINOR >= 8)
  #define UPCRT_STARTUP_SHALLOC(sptr, blockbytes, numblocks, mult_by_threads, elemsz, typestr) \
      { &(sptr), (blockbytes), (numblocks), (mult_by_threads), (elemsz), #sptr, (typestr) }
#else
  #define UPCRT_STARTUP_SHALLOC(sptr, blockbytes, numblocks, mult_by_threads, elemsz, typestr) \
      { &(sptr), (blockbytes), (numblocks), (mult_by_threads) }
#endif
#define UPCRT_STARTUP_PSHALLOC UPCRT_STARTUP_SHALLOC

/**** Autonb optimization ********/

extern void _upcrt_memput_nb(upcr_shared_ptr_t _dst, const void *_src, size_t _n);
#define upcrt_memput_nb(dst, src, n) \
       (upcri_srcpos(), _upcrt_memput_nb(dst, src, n))

#endif


/* Types */
/* File-level vars and routines */
extern void print_vec(upcr_pshared_ptr_t);

extern int thread_id(int);

extern int user_main(int, char **);


#define UPCR_SHARED_SIZE_ 8
#define UPCR_PSHARED_SIZE_ 8
upcr_pshared_ptr_t veca;
upcr_pshared_ptr_t vecb;
upcr_pshared_ptr_t vecc;
upcr_pshared_ptr_t time_diffs;

void UPCRI_ALLOC_main_221775937(void) { 
UPCR_BEGIN_FUNCTION();
upcr_startup_pshalloc_t _bupc_pinfo[] = { 
UPCRT_STARTUP_PSHALLOC(veca, 8, 1000000, 0, 8, "A1000000_R1_d"), 
UPCRT_STARTUP_PSHALLOC(vecb, 8, 1000000, 0, 8, "A1000000_R1_d"), 
UPCRT_STARTUP_PSHALLOC(vecc, 8, 1000000, 0, 8, "A1000000_R1_d"), 
UPCRT_STARTUP_PSHALLOC(time_diffs, 8, 4, 0, 8, "A4_R1_d"), 
 };

UPCR_SET_SRCPOS("_main_221775937_ALLOC",0);
upcr_startup_pshalloc(_bupc_pinfo, sizeof(_bupc_pinfo) / sizeof(upcr_startup_pshalloc_t));
}

void UPCRI_INIT_main_221775937(void) { 
UPCR_BEGIN_FUNCTION();
UPCR_SET_SRCPOS("_main_221775937_INIT",0);
}

#line 29 "main.upc"
extern void print_vec(
  upcr_pshared_ptr_t vec)
#line 29 "main.upc"
{
#line 29 "main.upc"
  UPCR_BEGIN_FUNCTION();
  
  UPCR_EXIT_FUNCTION();
  return;
} /* print_vec */


#line 44 "main.upc"
extern int thread_id(
  int i)
#line 44 "main.upc"
{
#line 44 "main.upc"
  UPCR_BEGIN_FUNCTION();
  
#line 45 "main.upc"
  UPCR_EXIT_FUNCTION();
#line 45 "main.upc"
  return i;
} /* thread_id */


#line 49 "main.upc"
extern int user_main(
  int argc,
  char ** argv)
#line 49 "main.upc"
{
#line 49 "main.upc"
  UPCR_BEGIN_FUNCTION();
  register _INT32 _bupc_comma;
  register _IEEE64 _bupc_comma0;
  int i;
  int run;
  struct timespec start;
  _INT32 _bupc_Tthreads0;
  struct timespec end;
  _IEEE64 _bupc___save_expr1;
  int thread;
  upcr_pshared_ptr_t _bupc_Mptra0;
  _IEEE64 _bupc_spillstoreparm1;
  upcr_pshared_ptr_t _bupc_Mptra2;
  _IEEE64 _bupc_spillld3;
  upcr_pshared_ptr_t _bupc_Mptra4;
  upcr_pshared_ptr_t _bupc_Mptra5;
  _IEEE64 _bupc_spillld6;
  upcr_pshared_ptr_t _bupc_Mptra7;
  _IEEE64 _bupc_spillld8;
  upcr_pshared_ptr_t _bupc_Mptra9;
  _IEEE64 _bupc_spillstoreparm10;
  upcr_pshared_ptr_t _bupc_Mptra11;
  _IEEE64 _bupc_spillld12;
  upcr_pshared_ptr_t _bupc_Mptra13;
  _IEEE64 _bupc_spillld14;
  upcr_pshared_ptr_t _bupc_Mptra15;
  _IEEE64 _bupc_spillstoreparm16;
  upcr_pshared_ptr_t _bupc_Mptra17;
  upcr_pshared_ptr_t _bupc_Mptra18;
  _IEEE64 _bupc_spillld19;
  upcr_pshared_ptr_t _bupc_Mptra20;
  _IEEE64 _bupc_spillld21;
  
#line 58 "main.upc"
  if(((int) upcr_mythread () ) == 0)
#line 58 "main.upc"
  {
#line 59 "main.upc"
    i = 0;
#line 59 "main.upc"
    while(i <= 999999)
#line 59 "main.upc"
    {
#line 60 "main.upc"
      _bupc_Mptra0 = UPCR_ADD_PSHARED1(vecb, 8ULL, i);
      _bupc_spillstoreparm1 = (_IEEE64)(i);
#line 60 "main.upc"
      UPCR_PUT_PSHARED(_bupc_Mptra0, 0, &_bupc_spillstoreparm1, 8);
#line 60 "main.upc"
      _bupc_Mptra4 = UPCR_ADD_PSHARED1(veca, 8ULL, i);
#line 60 "main.upc"
      _bupc_Mptra2 = UPCR_ADD_PSHARED1(vecb, 8ULL, i);
#line 60 "main.upc"
      _bupc_spillld3 = UPCR_GET_PSHARED_DOUBLEVAL(_bupc_Mptra2, 0);
#line 60 "main.upc"
      UPCR_PUT_PSHARED(_bupc_Mptra4, 0, &_bupc_spillld3, 8);
#line 60 "main.upc"
      _1 :;
#line 60 "main.upc"
      i = i + 1;
    }
#line 62 "main.upc"
    print_vec(veca);
#line 63 "main.upc"
    print_vec(vecb);
  }
#line 66 "main.upc"
  upcr_barrier(221775937, 1);
#line 68 "main.upc"
  run = 0;
#line 68 "main.upc"
  while(run <= 2)
#line 68 "main.upc"
  {
#line 69 "main.upc"
    get_time_nsec(&start);
#line 99 "main.upc"
    if(upcr_forall_control)
#line 99 "main.upc"
    {
#line 99 "main.upc"
      i = 0;
#line 99 "main.upc"
      while(i <= 999999)
      {
#line 100 "main.upc"
        _bupc_Mptra9 = UPCR_ADD_PSHARED1(vecc, 8ULL, i);
#line 100 "main.upc"
        _bupc_Mptra5 = UPCR_ADD_PSHARED1(veca, 8ULL, i);
#line 100 "main.upc"
        _bupc_spillld6 = UPCR_GET_PSHARED_DOUBLEVAL(_bupc_Mptra5, 0);
#line 100 "main.upc"
        _bupc_Mptra7 = UPCR_ADD_PSHARED1(vecb, 8ULL, i);
#line 100 "main.upc"
        _bupc_spillld8 = UPCR_GET_PSHARED_DOUBLEVAL(_bupc_Mptra7, 0);
        _bupc_spillstoreparm10 = _bupc_spillld6 + _bupc_spillld8;
#line 100 "main.upc"
        UPCR_PUT_PSHARED(_bupc_Mptra9, 0, &_bupc_spillstoreparm10, 8);
#line 100 "main.upc"
        _27 :;
#line 100 "main.upc"
        i = i + 1;
      }
    }
    else
#line 99 "main.upc"
    {
#line 99 "main.upc"
      upcr_forall_control = 1;
#line 99 "main.upc"
      i = 0;
#line 99 "main.upc"
      while(i <= 999999)
#line 99 "main.upc"
      {
        _bupc_Tthreads0 = upcr_threads();
        _bupc_comma = upcr_mythread();
        if(_I4MOD(i, _bupc_Tthreads0) == _bupc_comma)
        {
#line 100 "main.upc"
          _bupc_Mptra15 = UPCR_ADD_PSHARED1(vecc, 8ULL, i);
#line 100 "main.upc"
          _bupc_Mptra11 = UPCR_ADD_PSHARED1(veca, 8ULL, i);
#line 100 "main.upc"
          _bupc_spillld12 = UPCR_GET_PSHARED_DOUBLEVAL(_bupc_Mptra11, 0);
#line 100 "main.upc"
          _bupc_Mptra13 = UPCR_ADD_PSHARED1(vecb, 8ULL, i);
#line 100 "main.upc"
          _bupc_spillld14 = UPCR_GET_PSHARED_DOUBLEVAL(_bupc_Mptra13, 0);
          _bupc_spillstoreparm16 = _bupc_spillld12 + _bupc_spillld14;
#line 100 "main.upc"
          UPCR_PUT_PSHARED(_bupc_Mptra15, 0, &_bupc_spillstoreparm16, 8);
        }
#line 100 "main.upc"
        _3 :;
#line 100 "main.upc"
        i = i + 1;
      }
#line 99 "main.upc"
      upcr_forall_control = 0;
    }
#line 193 "main.upc"
    get_time_nsec(&end);
#line 194 "main.upc"
    _bupc_comma0 = time_diff_nsec(&start, &end);
#line 194 "main.upc"
    _bupc_Mptra17 = UPCR_ADD_PSHARED1(time_diffs, 8ULL, ((int) upcr_mythread () ));
    UPCR_PUT_PSHARED_DOUBLEVAL(_bupc_Mptra17, 0, _bupc_comma0);
#line 195 "main.upc"
    upcr_barrier(221775938, 1);
#line 196 "main.upc"
    _2 :;
#line 196 "main.upc"
    run = run + 1;
  }
#line 198 "main.upc"
  upcr_barrier(221775939, 1);
#line 200 "main.upc"
  if(((int) upcr_mythread () ) == 0)
#line 200 "main.upc"
  {
#line 201 "main.upc"
    print_vec(vecc);
#line 204 "main.upc"
    i = 0;
#line 204 "main.upc"
    while(i <= 999999)
#line 204 "main.upc"
    {
#line 205 "main.upc"
      _bupc___save_expr1 = (_IEEE64)(i);
#line 205 "main.upc"
      _bupc_Mptra18 = UPCR_ADD_PSHARED1(vecc, 8ULL, i);
#line 205 "main.upc"
      _bupc_spillld19 = UPCR_GET_PSHARED_DOUBLEVAL(_bupc_Mptra18, 0);
#line 205 "main.upc"
      if(_bupc_spillld19 != (_bupc___save_expr1 + _bupc___save_expr1))
#line 205 "main.upc"
      {
#line 206 "main.upc"
        printf("error\n");
#line 207 "main.upc"
        goto _5;
      }
#line 209 "main.upc"
      _4 :;
#line 209 "main.upc"
      i = i + 1;
    }
#line 209 "main.upc"
    printf("success\n");
#line 212 "main.upc"
    _5 :;
  }
#line 214 "main.upc"
  if(((int) upcr_mythread () ) == 0)
#line 214 "main.upc"
  {
#line 215 "main.upc"
    thread = 0;
#line 215 "main.upc"
    while(thread <= 3)
#line 215 "main.upc"
    {
#line 216 "main.upc"
      _bupc_Mptra20 = UPCR_ADD_PSHARED1(time_diffs, 8ULL, thread);
#line 216 "main.upc"
      _bupc_spillld21 = UPCR_GET_PSHARED_DOUBLEVAL(_bupc_Mptra20, 0);
#line 216 "main.upc"
      printf("T %2d: %.15f s\n", thread, _bupc_spillld21);
#line 216 "main.upc"
      _6 :;
#line 216 "main.upc"
      thread = thread + 1;
    }
  }
#line 218 "main.upc"
  UPCR_EXIT_FUNCTION();
#line 218 "main.upc"
  return 0;
} /* user_main */

#line 1 "_SYSTEM"
/**************************************************************************/
/* upcc-generated strings for configuration consistency checks            */

GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_GASNetConfig_gen, 
 "$GASNetConfig: (/tmp/upcc--19363-1328800726/main.trans.c) RELEASE=1.18.0,SPEC=1.8,CONDUIT=IBV(IBV-1.14/IBV-1.12),THREADMODEL=SEQ,SEGMENT=FAST,PTR=64bit,noalign,nopshm,nodebug,notrace,nostats,nosrclines,timers_native,membars_native,atomics_native,atomic32_native,atomic64_native $");
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_UPCRConfig_gen,
 "$UPCRConfig: (/tmp/upcc--19363-1328800726/main.trans.c) VERSION=2.14.0,PLATFORMENV=distributed,SHMEM=none,SHAREDPTRREP=packed/p21:t8:a35,TRANS=berkeleyupc,nodebug,nogasp,notv,staticthreads=4 $");
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_translatetime, 
 "$UPCTranslateTime: (main.o) Thu Feb  9 07:18:46 2012 $");
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_GASNetConfig_obj, 
 "$GASNetConfig: (main.o) " GASNET_CONFIG_STRING " $");
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_UPCRConfig_obj,
 "$UPCRConfig: (main.o) " UPCR_CONFIG_STRING UPCRI_THREADCONFIG_STR " $");
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_translator, 
 "$UPCTranslator: (main.o) /usr/local/upc/2.14.0/translator/install/targ (aphid) $");
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_upcver, 
 "$UPCVersion: (main.o) 2.14.0 $");
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_compileline, 
 "$UPCCompileLine: (main.o) /usr/local/upc/2.14.0/runtime/inst/bin/upcc.pl --at-remote-http --arch-size=64 --network=ibv -T 4 --lines --trans --sizes-file=upcc-sizes main.i $");
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_compiletime, 
 "$UPCCompileTime: (main.o) " __DATE__ " " __TIME__ " $");
#ifndef UPCRI_CC /* ensure backward compatilibity for http netcompile */
#define UPCRI_CC <unknown>
#endif
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_backendcompiler, 
 "$UPCRBackendCompiler: (main.o) " _STRINGIFY(UPCRI_CC) " $");

#ifdef GASNETT_CONFIGURE_MISMATCH
  GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_configuremismatch, 
   "$UPCRConfigureMismatch: (main.o) 1 $");
  GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_configuredcompiler, 
   "$UPCRConfigureCompiler: (main.o) " GASNETT_PLATFORM_COMPILER_IDSTR " $");
  GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_buildcompiler, 
   "$UPCRBuildCompiler: (main.o) " PLATFORM_COMPILER_IDSTR " $");
#endif

/**************************************************************************/
GASNETT_IDENT(UPCRI_IdentString_main_o_1328800726_transver_2,
 "$UPCTranslatorVersion: (main.o) 2.14.0, built on Oct 25 2011 at 16:40:27, host aphid linux-x86_64/64, gcc v4.2.4 (Ubuntu 4.2.4-1ubuntu4) $");

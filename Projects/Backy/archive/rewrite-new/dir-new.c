...
__attribute__((const))
char dirtype_toc(const DirType t) {
  switch(t) {
  case DIRTYPE_LNK: return 'L';
  case DIRTYPE_FIL: return 'F';
  case DIRTYPE_DIR: return 'D';
  case DIRTYPE_REM: return 'R';
  default:          return nul;
  }
}

__attribute__((const))
DirType dirtype_fromc(const char c) {
  switch(c) {
  case 'L': return DIRTYPE_LNK;
  case 'F': return DIRTYPE_FIL;
  case 'D': return DIRTYPE_DIR;
  case 'R': return DIRTYPE_REM;
  default:  return nul;
  }
}
...
int dir_snprintf(const bool is_action_list,
  ...
  return
    d->type == DIRTYPE_LNK ?
    snprintf(str, len, "%c%c%s%c%s",
             dirtype_toc(d->type), nul,
             d->path,              nul,
             d->info.target):
    d->type == DIRTYPE_FIL && is_action_list ?
    snprintf(str, len, "%c%c%s%c%lld",
             dirtype_toc(d->type), nul,
             d->path,              nul,
             d->info.finfo.size):
    d->type == dtype_file && !is_action_list ?
    snprintf(str, len, "%c%c%s%c%ld%c%lld",
             dtype_toc(d->type),  nul,
             d->path,             nul,
             d->info.finfo.mtime, nul,
             d->info.finfo.size):
    d->type == dtype_dir ?
    snprintf(str, len, "%c%c%s",
             dtype_toc(d->type), nul,
             d->path):
    d->type == daction_rem && is_action_list ?

  if (d->type == DIRTYPE_LNK)
    return snprintf(str, len, "%c%c%s%c%s",
                    dirtype_toc(d->type), nul,
                    d->path,              nul,
                    d->info.target);
  else if (d->type == DIRTYPE_FIL && is_action_list) {

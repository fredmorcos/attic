...
bool dir_load_pre(const struct dirent *const e, struct vec *const res) {
  ...
  *resp++ = NIL;
  ...
  if (strlcpy(resp, e->d_name, dname_size) >= dname_size) {
    errno = ENOMEM;
    return false;
  }
...
void *dir_load_a_cb(struct dir *const self) {
  assert(self);
  assert(self->name);
  self->ret = dir_load(self, self->name);
  return NULL;
}
...
bool dir_load_a(struct dir *const self) {
  assert(self);
  assert(self->name);
  errno = pthread_create(&self->thread, NULL, (pt_func *) dir_load_a_cb, self);
  self->thread_valid = (errno == 0);
  return self->thread_valid;
}

...
/* XXX: The lsdir_rec()  and tdir_rec() implementations
 * can seriously  be merged together. I  just could not
 * come up with an elegant way to do so.
 */

static Vec *tdir_rec(LsDir *const self, Vec *const res)
  attr((warn_unused_result, nonnull));

static Vec *lsdir_rec(LsDir *const self)
  attr((warn_unused_result, nonnull));

static Vec *lsdir_fail_name(LsDir *const self, const char *const name)
  attr((warn_unused_result, nonnull));

static Vec *lsdir_fail_errno(LsDir *const self)
  attr((warn_unused_result, nonnull));

Vec *tdir_rec(LsDir *const self, Vec *const res) {
  assert(self != NULL);
  ...
  Thread *const thread = (Thread *) self;
  ...
  while ((ent = readdir(dir))) {
    ...
    if (ent->d_type == DT_DIR) {
      ...
      resp->info.children = vec_init(sizeof(TDir), 32, (VecCB) tdir_free);
...
Vec *lsdir(LsDir *const self) {
  ...
  if ((res = lsdir_rec(self)) != NULL) {
    /* XXX: this variable is  volatile because the line
     * below  (*resp  =  nul)   is,  for  some  reason,
     * optimized  out on  GCC and  I cannot  figure out
     * why.
     */
...
inline Vec *lsdir_fail_name(LsDir *const self, const char *const name) {
  assert(self != NULL);
  assert(self->estr != NULL);
  assert(*self->estr == NULL);
  assert(name != NULL);
...
inline Vec *lsdir_fail_errno(LsDir *const self) {
  assert(self != NULL);
  self->sys_errno = errno;
  return (NULL);
}

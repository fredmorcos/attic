...
int main(int argc, char *argv[]) {
  ...
  {                       /* start loading directory */
    AutoVec dlist = vec_init(sizeof(TDir), 32, (VecCB) tdir_free);
    AutoStr estr = NULL;
    ...
    if (thread_init(lsdir_thread, (ThreadCB) tdir, &lsdir_info))
    ...
    {                                   /* receive dir list */
      ...
      AutoVec tmp_sdlist = vec_init(sizeof(char), chunk, NULL);
      int recv_rc = 0;
      ...
      {
        AutoStr hsize = humansize(tmp_sdlist.len);
      ...
      {
        AutoVec slist = vec_init(sizeof(TDir), 32, (VecCB) tdir_free);
        ...
        for (size_t i = 0; i < slist.len; i++)
          warnx("%c %s",
                (((TDir *) slist.ptr)[i]).type,
                (((TDir *) slist.ptr)[i]).filename);
      }
    }

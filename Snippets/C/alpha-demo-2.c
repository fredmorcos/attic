...
static gboolean expose (GtkWidget *widget,
						...
						gpointer user_data);
static void clicked	(GtkWindow *win,
					 GdkEventButton *event,
					 gpointer user_data);
static gboolean alphademo_slider_value_changed (GtkRange *range,
...
int main(int argc, char **argv)
{
	...
	g_signal_connect (G_OBJECT(slider),
					  "change-value",
					  G_CALLBACK(alphademo_slider_value_changed),
					  (gpointer) window);
    ...
    /* toggle title bar on click - we add the mask to tell X
												...
												gpointer the_window)
{
	...
	gtk_widget_queue_draw(GTK_WIDGET(the_window));
	g_print("DEBUG...\n");
	return FALSE;
}
